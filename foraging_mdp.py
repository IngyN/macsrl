"""Grid World MDP Implementation for foraging env
"""
import numpy as np
import importlib
...
from lbforaging.foraging import  Player, ForagingEnv, Action, CellEntity
...
# Up, Down, Right, Left
Actions = [Action.NONE, Action.NORTH, Action.SOUTH, Action.WEST, Action.EAST, Action.LOAD] 
...
class ForagingMDP():
    ...
    def __init__(self, shape, env, food_labels=True, structure=None, reward=None, label=None, A=Actions, p=1.0, figsize=6, lcmap={}, cmap=plt.cm.RdBu):
        self.shape = shape # TODO might need to change shape to include food info
        n_rows, n_cols = shape
        ...
        if food_labels:
            self.all_labels.append(['f', 'e'])

        print(f'all labels : {self.all_labels}')
    ...
    def get_transition_prob(self,state,action_name):
        ...
        if action_name == Action.NONE:
            return [state], np.array([1.])

        # Loading doesn't change location only label. 
        if action_name == Action.LOAD: 
            return [state], np.array([1.])
        ...
        # Movement
        # South
        if action_name!=Action.NORTH and state[0]+1 < n_rows and self.structure[state[0]+1][state[1]] != 'B' and cell_type != 'U':
            states.append((state[0]+1,state[1]))
            probs.append(self.p if action_name==Action.SOUTH else (1-self.p)/2)
        # North
        if action_name!=Action.SOUTH and state[0]-1 >= 0 and self.structure[state[0]-1][state[1]] != 'B' and cell_type != 'D':
            states.append((state[0]-1,state[1]))
            probs.append(self.p if action_name==Action.NORTH else (1-self.p)/2) 
        # West
        if action_name!=Action.EAST and state[1]-1 >= 0 and self.structure[state[0]][state[1]-1] != 'B' and cell_type != 'R':
            states.append((state[0],state[1]-1))
            probs.append(self.p if action_name==Action.WEST else (1-self.p)/2)
        # East
        if action_name!=Action.WEST and state[1]+1 < n_cols and self.structure[state[0]][state[1]+1] != 'B' and cell_type != 'L':
            states.append((state[0],state[1]+1))
            probs.append(self.p if action_name==Action.EAST else (1-self.p)/2)
