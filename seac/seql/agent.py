...
class Agent:
    ...
    def __init__(self, observation_size, action_size, params):
        ...
        self.model = QNetwork(observation_size, action_size, params.hidden_dim).to(device)
    ...
    def step(self, obs, explore=False, available_actions=None):
        ...
        if available_actions is not None:
            # assert self.discrete_actions #FIXME I'm not sure where this was supposed to be initialized
            available_mask = torch.ByteTensor(list(map(lambda a: a == 1, available_actions))).to(device)
