...
class IQL(MarlAlgorithm):
    ...
    def reset(self, episode):
        ...
        """
        self.prep_rollouts(dev=device)

    def prep_rollouts(self, dev="cuda"):
        """
        ...
        if dev == "cuda":
            ...
            fn = lambda x: x.cpu()
            print(f" cpu is here rollouts {dev}")
            
        if not self.model_dev == dev:
            for a in self.agents:
                a.model = fn(a.model)
            self.model_dev = dev
    ...
    def prep_training(self, dev="cuda"):
        """
        ...
        if dev == "cuda":
            ...
            fn = lambda x: x.cpu()
            print(f" cpu is here: {dev} - (dev==device):")
            print('\n'.join(ndiff(dev, "cuda")))
        ...
        if not self.model_dev == dev:
            for a in self.agents:
                a.model = fn(a.model)
            self.model_dev = dev
        if not self.trgt_model_dev == dev:
            for a in self.agents:
                a.target_model = fn(a.target_model)
            self.trgt_model_dev = dev
    ...
    def update_agent(self, sample, agent_i, use_cuda):
        ...
        target_next_states = (target_next_states).to(device)
        rews = (rews).to(device)
        ...
        # local Q-values
        acs = (acs).to(device)
        all_q_states = curr_agent.model(obs).to(device)
    ...
    def update(self, memory, use_cuda=True):
        """
